...
def create_annotation_directory(camera):
    home_dir ="C:/Users/rutvi/OneDrive/Desktop/Deep_Learning/Demo Project/Dataset/cadcd1/autonomoose_0022/processedmoose"
...
def write_txt_annotation(frame,cam,directory):
    ...
    img_path = "C:/Users/rutvi/OneDrive/Desktop/Deep_Learning/Demo Project/Dataset/cadcd1/autonomoose_0022/processedmoose/image_0" + cam + "/data/" + format(frame, '010') + ".png"
    print(img_path)
    if DISTORTED:
      path_type = 'raw'
    else:
      path_type = 'processedmoose'
    annotations_file = 'C:/Users/rutvi/OneDrive/Desktop/Deep_Learning/Demo Project/Dataset/cadcd1/autonomoose_0022/3d_annotations.json'; #contains all the annotations
    lidar_path = "C:/Users/rutvi/OneDrive/Desktop/Deep_Learning/Demo Project/Dataset/cadcd1/autonomoose_0022/" + path_type + "/lidar_points/data/" + format(frame, '010') + ".bin" #change this to snowy or not snowy
    calib_path = 'C:/Users/rutvi/OneDrive/Desktop/Deep_Learning/Demo Project/Dataset/cadcd1/autonomoose_0022/calibmoose'
    ...
    for cuboid in annotations_data[frame]['cuboids']:
      ...
      * .as_dcm - Represent as direction cosine matrices.
      ...
      if len(lidar_bag) > filter: #if we filter such that the annotations should have at least 20 lidar
          ...
          tmp_yaw = -R.from_matrix(T_CAM_CUBOID[0:3, 0:3]).as_euler('zyx', degrees=False)[0]
          ...
          '''
          coords1=[]
          coords1.append(round(x_min_set,2))
          coords1.append(round(y_min_set,2))
          coords1.append(round(x_max_set,2))
          coords1.append(round(y_max_set,2))

          x1 = float((float(coords1[2]) + float(coords1[0])) / 2.0) / float(img_w)
          y1 = float((float(coords1[3]) + float(coords1[1])) / 2.0) / float(img_h)
          width1 = float(float(coords1[2]) - float(coords1[0])) / float(img_w)
          height1 = float(float(coords1[3]) - float(coords1[1])) / float(img_h)
          label_list=['Car', 'Pedestrian', 'Truck', 'Bus','Garbage_Containers_on_Wheels','Traffic_Guidance_Objects','Bicycle','Pedestrian_With_Object','Horse_and_Buggy','Animal','Person_sitting', 'Cyclist', 'Tram', 'Misc','DontCare']



          f.write(' %s'%(label_list.index(cuboid['label'])))
          ...
          f.write(' %s %s %s %s ' %((x1), (y1), (width1), (height1)))
...
def write_all_annotations(cam):
    ...
    img_dir = "C:/Users/rutvi/OneDrive/Desktop/Deep_Learning/Demo Project/Dataset/cadcd1/autonomoose_0022/processedmoose/image_0" + cam + "/data"
